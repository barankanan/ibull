-- ============================================================================
--  GARSON GEÇMİŞ MASALAR — TABLO ve YENİDEN YAZDIRMA RPC'LERİ
--  ----------------------------------------------------------------------------
--  Bu dosyayı Supabase SQL Editor'a tek seferde yapıştırın ve çalıştırın.
--  Hatasız, idempotent (tekrar çalıştırılabilir) yazılmıştır.
--
--  Eklediği şeyler:
--    1) public.table_order_history tablosu (yoksa)
--    2) RLS politikaları + indeksler
--    3) create_kitchen_reprint_print_job RPC  → "(ESKİ MASA)" mutfak fişi
--    4) create_adisyon_reprint_print_job RPC  → "(ESKİ MASA)" adisyon
--
--  Not: user_can_access_restaurant() fonksiyonu daha önceki migration'larda
--       tanımlandığı için burada tekrar tanımlanmıyor.
-- ============================================================================

begin;

-- ── 1. table_order_history tablosu ──────────────────────────────────────────
...
begin
  ...
  select business_name into v_restaurant_name
    from public.stores where seller_id = p_restaurant_id limit 1;
  ...
  insert into public.print_jobs (
    restaurant_id, order_id, station_id, printer_id,
    job_type, document_type, printer_role, status, payload
  )
  values (
    p_restaurant_id, null, null, null,
    'reprint', 'kitchen', 'mutfak', 'pending', v_payload
  )
...
grant execute on function public.create_kitchen_reprint_print_job(
  uuid, integer, jsonb, uuid, text, text, uuid
) to authenticated;

-- ── 3. Adisyon yeniden basımı ──────────────────────────────────────────────
...
begin
  ...
  values (
    p_restaurant_id, null, null, null,
    'receipt', 'receipt', 'adisyon', 'pending', v_payload
  )
  returning id into v_job_id;
