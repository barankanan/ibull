#!/usr/bin/env python3
# Helper script — rewrites seller_campaign_table.dart with all UX changes.
import os

TARGET = os.path.join(
    os.path.dirname(__file__),
    "ibul_app/lib/ads/presentation/widgets/seller_campaign_table.dart",
)
...
class CampaignTableColumnDef {
  ...
  final _CellBuilder cellBuilder;
  final double width;
...
class _SellerCampaignTableState extends State<SellerCampaignTable> {
  ...
  // Column reorder state (user-draggable order).
  List<String>? _columnOrder;

  // Per-column width overrides from resize drags.
  ...
  // Uniform cell horizontal padding — tighter than before for Meta-like density
  static const double _cellPaddingH = 10.0;
  static const double _actionsWidth = 52.0;
  ...
  static String _fmtMoney(double v, String cur) {
    ...
    return cents == 0 ? '\$w \$cur' : '\$w,\${cents.toString().padLeft(2, "0")} \$cur';
  }
  ...
  // ─── Column definitions ───────────────────────────────────────────────────────
  // 'date' is listed first so that the default column order starts with it.
    ...
    return [
      ...
      CampaignTableColumnDef(
        ...
        cellBuilder: (ctx, row, c) {
          final v =
              row.metrics.spend > 0 ? row.metrics.spend : row.campaign.spentAmount;
          return Text(
            _fmtMoney(v, c),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
      ...
      // ── Always-last: actions (not reorderable) ──────────────────────────────
      CampaignTableColumnDef(
        id: 'actions',
        labelTr: '',
        labelEn: '',
        ...
        sortable: false,
        reorderable: false,
        cellBuilder: (ctx, row, _) => CampaignActionMenu(
          isPaused: row.campaign.status == CampaignStatus.paused,
          onDetail: widget.onDetail != null ? () => widget.onDetail!(row.campaign) : null,
          onEdit: widget.onEdit != null ? () => widget.onEdit!(row.campaign) : null,
          onPause: widget.onPause != null ? () => widget.onPause!(row.campaign) : null,
          onResume: widget.onResume != null ? () => widget.onResume!(row.campaign) : null,
          onDelete: widget.onDelete != null ? () => widget.onDelete!(row.campaign) : null,
        ),
      ),
    ];
  }
  ...
  static const _optionalIds = <String>{
    'favorites', 'add_to_carts', 'store_visits', 'conversions',
  };
  ...
  List<CampaignTableColumnDef> _orderedVisibleColumns(
    ...
    if (actions != null) result.add(actions);
    return result;
  }

  static CampaignTableColumnDef _withWidth(CampaignTableColumnDef col, double w) {
  ...
  void _syncOrderToProps(List<CampaignTableColumnDef> all) {
    ...
    if (pruned.length != _columnOrder!.length || !pruned.every(_columnOrder!.contains)) {
      _columnOrder = pruned;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bodyHeight =
        math.max(_bodyMin, math.min(_bodyMax, _rowHeight * widget.rows.length));
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Column(
          ...
          children: [
            _buildHeader(cols, totalWidth, all),
            SizedBox(
              height: bodyHeight,
              child: Scrollbar(
                controller: _vert,
    ...
    return Material(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [
            BoxShadow(color: Color(0x1A3B82F6), blurRadius: 8, offset: Offset(0, 4)),
          ],
        ),
        padding:
            const EdgeInsets.symmetric(horizontal: _cellPaddingH, vertical: 8),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment:
              col.numeric ? CrossAxisAlignment.end : CrossAxisAlignment.start,
          children: [
            Text(col.labelTr,
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
        ...
        if (col.labelTr.isNotEmpty)
          Text(
            ...
            style: TextStyle(
              fontSize: 10,
              fontWeight: FontWeight.w500,
              color: isSorted ? const Color(0xFF3B82F6) : const Color(0xFF94A3B8),
            ),
          ),
        const SizedBox(height: 3),
        Row(
          ...
          children: [
            Flexible(
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w800,
                  color: isSorted ? const Color(0xFF1D4ED8) : const Color(0xFF334155),
                ),
              ),
            ),
            if (col.sortable && widget.onSortChanged != null) ...[
              ...
              Icon(
                ...
                size: 11,
                color: isSorted ? const Color(0xFF2563EB) : const Color(0xFFCBD5E1),
              ),
            ],
          ],
        ),
        ...
        children: [
          Container(
            ...
            padding: const EdgeInsets.symmetric(
                horizontal: _cellPaddingH, vertical: 8),
            alignment:
          ...
          // Resize handle at the right edge of every header cell
          Positioned(
            right: 0,
            top: 0,
            bottom: 0,
            child: _ResizeHandle(
              ...
              onDragUpdate: (globalX) {
                ...
                final newW =
                    (_resizeDragStartWidth + delta).clamp(col.minWidth, 600.0);
  ...
  Widget _buildDataRow(
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children:
              cols.map((c) => _buildDataCell(ctx, row, c)).toList(growable: false),
        ),
      ),
    );
  }
  ...
  Widget _buildFooter() {
    ...
    final resolvedSize =
        pageSizes.contains(widget.pageSize) ? widget.pageSize : pageSizes.first;
    ...
    final end =
        (widget.pageIndex * widget.pageSize + widget.rows.length)
            .clamp(0, widget.totalRowCount);
...
// ─────────────────────────────────────────────────────────────────────────────
// Resize handle — isolated widget so its GestureDetector doesn't conflict with
// the header cell's Draggable.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        onHorizontalDragUpdate: (d) =>
            widget.onDragUpdate(d.globalPosition.dx),
