-- Hotfix: orders schema compatibility for kitchen print RPC.
--
-- Symptoms (client snackbar):
--   column "table_number" of relation "orders" does not exist
--
-- Root cause:
--   Live project uses a legacy implementation function
--   `public.create_table_order_with_print_jobs_impl(...)` (introduced by
--   20260501_waiter_phone_access_print_rpcs.sql wrapper). That legacy impl
--   attempted to insert/update orders.table_number (or related fields) even
--   though public.orders does not have such columns in production.
--
-- Fix strategy:
-- - Keep the waiter-access wrapper `public.create_table_order_with_print_jobs(...)`
--   intact (so sub-admin permission logic remains).
-- - Replace ONLY the legacy impl with the repo-canonical implementation that:
--   - does NOT insert any table label fields into `public.orders`
--   - carries table label metadata inside `print_jobs.payload`
--   - avoids the v_item alias FROM-clause bug by treating v_item as a loop record
--
-- Safety note:
--   Do not insert table_number/table_name/display_table_label/table_area_name
--   into `public.orders` unless the columns exist. Table labels are print payload
--   metadata, not order columns.
--
-- After deployment, refresh PostgREST schema cache:
--   notify pgrst, 'reload schema';

-- 1) Replace the legacy implementation (owner-context).
...
declare
  ...
  v_job_payload jsonb;
begin
  -- Owner-context guard (legacy behaviour): caller is impersonated as restaurant id by wrapper.
  ...
  -- unless the columns exist. Table label fields are print payload metadata.
  ...
  -- Station grouping + print job creation.
  for v_group in
    select
      oi.station_id,
...
-- 2) Ensure wrapper continues to exist and points to _impl.
-- (Re-apply wrapper from 20260501 in case deployments drifted.)
