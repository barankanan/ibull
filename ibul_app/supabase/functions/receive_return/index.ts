...
type RequestBody = { return_id: string; mark_shipped_back?: boolean; note?: string };

Deno.serve(async (req) => {
  try {
    const { user, serviceClient } = await getAuthedContext(req);
    const body = await parseJson<RequestBody>(req);
    const data = await callRpc<Record<string, unknown>>(serviceClient, 'seller_ops.receive_order_return', {
      p_actor_user_id: user.id,
      p_return_id: body.return_id,
      p_mark_shipped_back: body.mark_shipped_back ?? false,
