...
const handlers = {
  ...
  'pre-bash': () => {
    ...
    const hookEvent = hookInput.hook_event_name || hookInput.hookEventName || 'PreToolUse';
    const emitDecision = (decision, reason) => {
    ...
    for (const d of dangerous) {
      if (cmd.includes(d)) {
        emitDecision('deny', `Dangerous command detected: ${d}`);
        return;
      }
    }
    if (stdinData.trim() || hookInput.command || toolInput.command) {
      emitDecision('allow');
      return;
    }
  ...
  'pre-edit': () => {
    if (stdinData.trim()) {
      console.log(JSON.stringify({
        hookSpecificOutput: {
          hookEventName: hookInput.hook_event_name || hookInput.hookEventName || 'PreToolUse',
